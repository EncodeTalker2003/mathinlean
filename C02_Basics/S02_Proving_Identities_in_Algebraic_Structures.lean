...
theorem mul_right_inv (a : G) : a * a⁻¹ = 1 := by
  rw [← mul_left_inv (a⁻¹)]
  nth_rw 3 [← one_mul a⁻¹]
  rw [← mul_left_inv a, ← mul_assoc, ← mul_assoc, mul_left_inv, one_mul]

theorem mul_one (a : G) : a * 1 = a := by
  rw [← mul_left_inv a, ← mul_assoc, mul_right_inv, one_mul]

theorem mul_inv_rev (a b : G) : (a * b)⁻¹ = b⁻¹ * a⁻¹ := by
  rw [← one_mul (b⁻¹ * a⁻¹), ← mul_left_inv (a * b), mul_assoc, mul_assoc, ← mul_assoc b, mul_right_inv, one_mul, mul_right_inv, mul_one]
